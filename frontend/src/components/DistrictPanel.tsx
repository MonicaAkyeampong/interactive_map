  ...
  return (
    <div className="w-56 bg-white/97 rounded-xl flex flex-col shadow-sm">
      ...
      <div className="flex items-center gap-1.5 px-2.5 py-1.5 border-b border-gray-100 bg-gray-50/50 rounded-t-xl">
        ...
        <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wide truncate" title={districtName}>{districtName}</h3>
      </div>
              ...
              <>
                {topSector && (
                  <div className="flex justify-between items-center text-[10px]">
                    ...
                    <span className="font-bold text-gray-800 max-w-[80px] truncate" title={topSector.sector}>{topSector.sector} <span className="text-[9px] text-gray-400 font-normal">({topSectorPct.toFixed(1)}%)</span></span>
                  </div>
                )}
                {topGas && (
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-gray-500 font-medium">Top Gas</span>
                    <span className="font-bold text-gray-800 max-w-[80px] truncate" title={topGas.gas}>{topGas.gas} <span className="text-[9px] text-gray-400 font-normal">({topGasPct.toFixed(1)}%)</span></span>
                  </div>
                )}
                {!topSector && !topGas && (
